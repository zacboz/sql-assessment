...
Create a table called Vehicle that has an id, make, model, year, and ownerId (a foreign key to users)

* Insert 6 records into Vehicle.  Below you find the make, model, year, and ownerId (for the foreign key)
...
* Create an endpoint at `GET '/api/user/:userId/vehiclecount'` that will return a count of how many vehicles belong to the given user  
  Response should be an object with a count property {count:1}
